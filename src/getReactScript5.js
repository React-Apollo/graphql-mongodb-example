...
/**
 ...
 重构版本-解决问题，从十几页去的数据flatten后一次插入数据库，可能会有问题
 应该是取一页的数据就插入数据库，再取下一页。数据库压力小
 **/
...
export const start = async () => {
  try {
    ...
    for (var i = 1; i <= 2; i++) {
       const res = await SinglePageDataInsertToDataBase(i);
       console.log(res);
     }
...
const SinglePageDataInsertToDataBase = compose(log, insertDataWaitForData, getArray, getDataReactScriptWaitForUrl, Resolvevar, variablesTemp)
